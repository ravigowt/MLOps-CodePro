...
def extracted_from_csv_into_db(conn):
...
def load_data_into_db():
    ...
    try:
        ...
        if not check_if_table_has_value(conn, "loaded_data"):
            extracted_from_csv_into_db(conn)
        else:
...
# ##############################################################################
def extracted_from_map_city_tier(conn):
...
def map_city_tier():
    ...
    try:
        ...
        if not check_if_table_has_value(conn, "city_tier_mapped"):
            extracted_from_map_city_tier(conn)
        else:
...
def extracted_from_map_categorical_vars(conn):
...
def map_categorical_vars():
    ...
    try:
        ...
        if not check_if_table_has_value(conn, "categorical_variables_mapped"):
            extracted_from_map_categorical_vars(conn)
        else:
...
# #############################################################################
def extracted_from_interactions_mapping(conn):
    ...
    trimmed_dataset = df_pivot[INDEX_COLUMNS_TRAINING[1:]]
...
def interactions_mapping():
    ...
    try:
        ...
        if not check_if_table_has_value(
            ...
            extracted_from_interactions_mapping(conn)
        else:
