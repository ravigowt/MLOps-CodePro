...
def test_load_data_into_db():
    ...
    connection.close()
    assert len(df.columns) == len(ref_df.columns), "loaded_data does not match loaded_data_test_case"
...
def test_map_city_tier():
    ...
    connection.close()
    assert len(df.columns) == len(ref_df.columns), "city_tier_mapped does not match with city_tier_mapped_test_case"
...
def test_map_categorical_vars():
    ...
    connection.close()
    assert len(df.columns) == len(ref_df.columns), "categorical_variables_mapped does not match with categorical_variables_mapped_test_case"
...
def test_interactions_mapping():
    ...
    connection.close()
    assert len(df.columns) == len(ref_df.columns), "interactions_mapped does not match with interactions_mapped_test_case"
